...
const SuppliersPage = () => {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const raw = Array.isArray(payload.suppliers) ? payload.suppliers : Array.isArray(payload) ? payload : [];
  ...
  const openCreate = () => { setEditItem(null); setForm({ name: '', company: '', phone: '', email: '', address: '', notes: '' }); setFormErrors({}); setShowModal(true); };
  const openEdit = (s) => { setEditItem(s); setForm({ name: s.name || '', company: s.company || '', phone: s.phone || '', email: s.email || '', address: s.address || '', notes: s.notes || '' }); setFormErrors({}); setShowModal(true); };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editItem) { await supplierService.update(editItem._id || editItem.id, form); toast.success('Supplier updated'); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setDeleting(true);
    try { await supplierService.delete(deleteId); toast.success('Supplier deleted'); setDeleteId(null); fetchSuppliers(); }
  ...
  return (
    <div className="page-container">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Supplier' : 'New Supplier'} size="lg">
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} loading={deleting} title="Delete Supplier" message="Are you sure you want to delete this supplier?" confirmLabel="Delete" variant="danger" />
    </div>
  );
};
